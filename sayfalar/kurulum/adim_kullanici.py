...
from sayfalar.ortak.bilesenler import alan, aciklama, buyuk_harfle_yazdir, degisince_hatayi_temizle
...
def kullanici_adimini_olustur(stepper: ui.stepper, smtp_verisi: dict, firma_adi: str) -> None:
    with ui.step("Sistem Yöneticisi"):
        ...
        with ui.row().classes("w-full gap-2 no-wrap"):
            sicil = alan("Sicil No *", "20260001",        genislik="flex-1")
            email = alan("E-posta *",  "admin@firma.com", genislik="flex-1")
        with ui.row().classes("w-full gap-2 no-wrap"):
            ad    = alan("Ad *",    genislik="flex-1")
            soyad = alan("Soyad *", genislik="flex-1")
        buyuk_harfle_yazdir(ad)
        buyuk_harfle_yazdir(soyad)
        with ui.row().classes("w-full gap-2 no-wrap"):
            parola        = alan("Parola *",        password=True, genislik="flex-1")
            parola_tekrar = alan("Parola Tekrar *", password=True, genislik="flex-1")
        parola_kurallarini_goster(parola, parola_tekrar)
        hata = ui.label("").classes("text-negative text-caption")
        degisince_hatayi_temizle(hata, sicil, email, ad, soyad, parola, parola_tekrar)
